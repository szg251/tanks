defmodule TanksWeb.GameChannel do
  ...
  def join("game:lobby", _payload, socket) do
    Battle.create_tank(socket.assigns.game_pid, socket.assigns.user_id)
  ...
  def handle_in("move", %{"move" => velocity}, socket) do
    {:ok, tankPid} = Battle.get_pid(socket.assigns.game_pid, socket.assigns.user_id)
  ...
  def handle_in("fire", %{}, socket) do
    Battle.fire(socket.assigns.game_pid, socket.assigns.user_id)
    {:noreply, socket}
  end

  def handle_info({:broadcast, prev_game_state}, socket) do
    game_state = Battle.get_state(socket.assigns.game_pid) |> Battle.to_api()
