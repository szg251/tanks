defmodule TanksWeb.GameChannel do
  ...
  def join("game:lobby", _payload, socket) do
    GameState.create_tank(socket.assigns.game_pid, socket.assigns.user_id)
  ...
  def handle_in("move", %{"move" => velocity}, socket) do
    {:ok, tankPid} = GameState.get_pid(socket.assigns.game_pid, socket.assigns.user_id)
  ...
  def handle_in("fire", %{}, socket) do
    GameState.fire(socket.assigns.game_pid, socket.assigns.user_id)
    {:noreply, socket}
  end

  def handle_info({:broadcast, prev_game_state}, socket) do
    game_state = GameState.get_state(socket.assigns.game_pid) |> GameState.to_api()
